...
# =============================================================================
# test-affine.sh
# Automated checks against a running AFFiNE deployment.
# Run from the repo root. Reads config from terraform output when the
# terraform/ directory exists; falls back to config/bootstrap-outputs.env
# for the legacy shell-script deployment.
#
# Usage:
#   ./scripts/test-affine.sh [--host <IP_or_domain>] [--skip-tls-verify]
# =============================================================================
set -eo pipefail
...
if [ -d "$REPO_ROOT/terraform" ] && command -v terraform &>/dev/null; then
  if terraform -chdir="$REPO_ROOT/terraform" output -json &>/dev/null 2>&1; then
    HOST="${HOST:-$(terraform -chdir="$REPO_ROOT/terraform" output -raw public_ip 2>/dev/null || echo "")}"
    KEY_FILE="$(terraform -chdir="$REPO_ROOT/terraform" output -raw ssh_key_path 2>/dev/null || echo "")"
    S3_BUCKET="$(terraform -chdir="$REPO_ROOT/terraform" output -raw s3_bucket 2>/dev/null || echo "")"
    EC2_USER="$(terraform -chdir="$REPO_ROOT/terraform" output -raw ec2_user 2>/dev/null || echo "ec2-user")"
  fi
fi

# Fall back to legacy env file
ENV_FILE="$REPO_ROOT/config/bootstrap-outputs.env"
...
if [ -z "$HOST" ]; then
  ...
  echo ""
  echo "Could not auto-detect host. Either:"
  echo "  - Run from the repo root after 'terraform apply'"
  echo "  - Pass --host <IP_or_domain> explicitly"
  exit 1
fi

# ── SSH setup for Docker/PostgreSQL/Redis checks ──────────────────────────────
...
echo ""
echo "========================================================"
echo " AFFiNE — Automated Test Suite"
echo " Target : $BASE"
...
if command -v aws &>/dev/null && [ -n "$S3_BUCKET" ]; then
  ...
  if aws s3 cp /dev/stdin "s3://${S3_BUCKET}/${TEST_KEY}" \
    ...
    echo -e "  \033[1;31m[FAIL]\033[0m S3 write failed — check IAM permissions"
    ((FAIL++))
  fi
else
  echo -e "  \033[1;33m[SKIP]\033[0m AWS CLI not available or S3 bucket unknown"
  ((WARN++))
fi
...
if [ $FAIL -eq 0 ]; then
  echo -e "\033[1;32mAll checks passed! AFFiNE is operational.\033[0m"
  echo ""
